...
export default function ThankYouPllaRf() {
  const { locale = "en" } = useRouter();
  const { t } = useTranslation("thankyou");
  const title = `${t("shared.title")} | Mave Medical Spa`;
  const description = t("sculptraRf.subtitle");
  const canonical = `https://www.mavemedspa.com${locale === "es" ? "/es" : ""}/thank-you-plla-rf`;
  const bodyLines = [t("sculptraRf.bodyLine1"), t("sculptraRf.bodyLine2")];
    ...
    <>
      ...
      <main className="bg-white pt-36 md:pt-40">
        <section className="max-w-3xl mx-auto px-6 py-16 text-center">
          ...
          <p className="text-base text-gray-700 mt-3">{t("sculptraRf.subtitle")}</p>

...
import { optionHasPromo } from "./promo";
...
const getLinkedPackageGroupsForTreatment = (treatment, locale) => {
  if (!treatment) return [];
  const activePromoOptions = (treatment?.pricing?.options || []).filter((opt) =>
    optionHasPromo(opt, locale)
  );
...
const getPackageGroupsForTreatment = (treatment, locale) => {
  ...
  const activePromoPackageIds = new Set(
    activePromoOptions.flatMap((opt) => {
      const ids = [];
  ...
  if (activePromoPackageIds.size === 0) return [];

  const sourcePackageIds = Array.isArray(treatment?.packages)
    ? treatment.packages
        .map((pkg) => pkg.packageId)
        .filter((packageId) => packageId && activePromoPackageIds.has(packageId))
    : [];
  const linkedPackageIds = Array.from(
    new Set(
      activePromoOptions
  ...
  const relatedPackageIds = currentServiceId
    ? Array.from(registry.values())
        .filter((entry) => {
          ...
          if (!activePromoPackageIds.has(entry.packageId)) return false;
